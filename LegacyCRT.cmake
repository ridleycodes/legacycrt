...
FUNCTION(__LEGACY_CRT_ADD_CRT_LIBRARY CRT_SRC_LOCATION)
    ...
    IF(CRT_SEH)
        SET_TARGET_PROPERTIES(legacy_crt_library_std PROPERTIES
            CRT_SEH "${CRT_SRC_LOCATION}/seh.lib")
    ENDIF(CRT_SEH)
...
MACRO(ADD_LEGACY_CRT CRT_LOCATION)
    IF(NOT DEFINED __LEGACY_CRT_INITIALIZED)
        ...
        ENDIF(NOT WIN32)
        IF(NOT CMAKE_CROSSCOMPILING AND NOT DEFINED CMAKE_LIB_COMPILER)
            IF(MSVC)
                SET(CMAKE_LIB_COMPILER "lib.exe")
            ELSE(MSVC)
                SET(CMAKE_LIB_COMPILER "dlltool")
            ENDIF(MSVC)
        ENDIF(NOT CMAKE_CROSSCOMPILING AND NOT DEFINED CMAKE_LIB_COMPILER)
...
FUNCTION(ADD_LEGACY_CRT_EXECUTABLE EXECUTABLE_NAME IS_CONSOLE ...)
    ...
    GET_TARGET_PROPERTY(CRT_SEH legacy_crt_library_std CRT_SEH)
    IF(CRT_SEH)
        TARGET_LINK_LIBRARIES(${EXECUTABLE_NAME} "${CRT_SEH}")
    ENDIF(CRT_SEH)
...
FUNCTION(ADD_LEGACY_CRT_DLL DLL_NAME IS_CONSOLE ...)
    ...
    GET_TARGET_PROPERTY(CRT_SEH legacy_crt_library_std CRT_SEH)
    IF(CRT_SEH)
        TARGET_LINK_LIBRARIES(${DLL_NAME} "${CRT_SEH}")
    ENDIF(CRT_SEH)
...
FUNCTION(ADD_LEGACY_CRT_LIBRARY LIBRARY_NAME IS_CONSOLE ...)
    ...
    GET_TARGET_PROPERTY(CRT_SEH legacy_crt_library_std CRT_SEH)
    IF(CRT_SEH)
        TARGET_LINK_LIBRARIES(${LIBRARY_NAME} "${CRT_SEH}")
    ENDIF(CRT_SEH)
